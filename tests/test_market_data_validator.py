...
class TestVerifiedSnapshot:
    ...
    def test_cn_a_stock_failure_falls_back_to_yfinance(self):
        # The snapshot path must fall back to yfinance when a_stock fails,
        # same as get_stock_data — not crash. Regression for the old hardcoded
        # cn->a_stock behavior that had no fallback.
        config_module._config = copy.deepcopy(default_config.DEFAULT_CONFIG)
        config_module._config["market_vendors"]["cn"]["core_stock_apis"] = "a_stock,yfinance"
        try:
            data = _sample_ohlcv()
            with (
                mock.patch(
                    "tradingagents.dataflows.a_stock._load_ohlcv_astock",
                    side_effect=NoMarketDataError("600519", "600519", "mootdx down"),
                ),
                mock.patch(
                ...
                snap = validator.build_verified_market_snapshot("600519", "2026-05-13")
